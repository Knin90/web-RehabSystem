    ...
    try:
        ...
        with app.app_context():
            try:
                ...
                print(f"✗ Error en operaciones de BD: {str(inner_e)}")
                import traceback
                traceback.print_exc()
                db.session.rollback()
                ...
                try:
                    db.create_all()
                    print("✓ Tablas creadas (sin datos iniciales)")
                except:
                    pass
                
    except Exception as e:
        print(f"✗ Error crítico al inicializar: {str(e)}")
        import traceback
        traceback.print_exc()
        # No fallar, continuar con la aplicación
        print("Continuando sin inicialización de BD...")
        sys.exit(0)  # Exit 0 para no detener el deploy
